...
# 目标镜像文件
TGT_IMG="${WORK_DIR}/${BOARD^}-LEDE-${OPENWRT_VER}-${BUILD_DATE}.img"
...
sed -i -e '/> $LOG/d' -e '/CoreMark/a\echo "\( CPU CoreMark : $COREMARK \)" > $LOG' "${COREMARK}"
sed -i -e '/ROOT1=/c\ROOT1=\"512\"' -e '/ROOT2=/c\ROOT2=\"512\"' ${OPENWRT_INSTALL}
...
adjust_getty_config
adjust_samba_config
[ -f ./etc/config/samba4 ] && sed -i 's/nas/services/g' ./usr/lib/lua/luci/controller/samba4.lua
...
sed -i -e '896 s/^/<!--/' -e '904 s/\$/-->/' ./usr/lib/lua/luci/view/admin_status/index.htm
adjust_kernel_env
copy_uboot_to_fs
write_release_info
# write_banner
