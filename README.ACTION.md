...
| KERNEL_REPO_URL        | [breakings/.../opt](openwrt_flippy.sh#L47) | 设置内核下载地址，默认从 breakings 维护的 [kernel](https://github.com/breakings/OpenWrt/tree/main/opt) 库里下载 Flippy 的内核。 |
| KERNEL_VERSION_DIR     | kernel_rk3588         | 设置内核下载目录。通用内核目录_rk3588内核目录 |
| KERNEL_VERSION_NAME    | 5.15.95_6.1.15        | 设置内核版本，[kernel](https://github.com/breakings/OpenWrt/tree/main/opt/kernel) 库里收藏了众多 Flippy 的原版内核，可以查看并选择指定。可指定单个内核如 `5.15.95` ，可选择多个内核用`_`连接如 `5.15.95_6.1.15` ，内核名称以 kernel 目录中的文件夹名称为准。 |
