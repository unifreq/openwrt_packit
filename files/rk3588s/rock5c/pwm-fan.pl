...
################################################################################
# Radxa Rock5C/Lite 风扇参数调整区
# 已按照 Radxa 官方提供的风扇进行适配，如果更换其它风扇，需要按实际情况调整参数
# 速度最小值(满速是99),如果小于10可能进入死区，风扇不转
my $speed_min = 5;
...
sub init {
    ...
    open $fh, ">", "/sys/class/pwm/${pwmchip}/export";
    print $fh "0\n";
    close $fh;

    open $fh, ">", "/sys/class/pwm/${pwmchip}/pwm0/period";
    print $fh "$period\n";
    close $fh;

    open $fh, ">", "/sys/class/pwm/${pwmchip}/pwm0/polarity";
    print $fh "normal\n";
    close $fh;

    open $fh, ">", "/sys/class/pwm/${pwmchip}/pwm0/enable";
    print $fh "1\n";
    close $fh;
}
...
sub set_fixed_speed {
    ...
    open my $fh, ">", "/sys/class/pwm/${pwmchip}/pwm0/duty_cycle";
    print $fh "$duty_cycle\n";
    close $fh;
}
